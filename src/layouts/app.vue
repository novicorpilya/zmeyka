<template>
  <div class="min-h-screen bg-[#F8FAFC] flex font-nunito overflow-hidden">
    <!-- Desktop Sidebar Navigation (Hidden on Mobile) -->
    <aside
      class="hidden md:flex w-24 lg:w-72 bg-white border-r-4 border-slate-100 flex-col relative z-50 transition-all duration-500 shrink-0"
    >
      <!-- Logo -->
      <div class="p-6 lg:p-10 flex items-center justify-center lg:justify-start gap-4">
        <NuxtLink to="/dashboard" class="flex items-center gap-3 group">
          <div
            class="w-12 h-12 bg-brand-green rounded-2xl flex items-center justify-center shadow-cartoon transition-transform group-hover:scale-110"
          >
            <img src="~/assets/logo.png" alt="Logo" class="w-8 h-8 object-contain" />
          </div>
          <span
            class="hidden lg:block text-2xl font-black text-slate-800 uppercase tracking-tighter"
            >Zmeyka</span
          >
        </NuxtLink>
      </div>
      ...
      <nav class="flex-grow px-4 lg:px-6 space-y-2 py-6 overflow-y-auto custom-scrollbar">
        ...
        >
          ...
          <div
            ...
            class="absolute left-0 top-1/4 bottom-1/4 w-1 bg-white rounded-r-full"
          ></div>
        </NuxtLink>
      </nav>
      ...
      <div class="p-4 lg:p-6 mt-auto border-t-4 border-slate-50 space-y-4">
        <button
          @click="handleLogout"
          class="w-full flex items-center justify-center lg:justify-start gap-4 p-4 rounded-2xl font-black text-red-400 hover:bg-red-50 hover:text-red-500 transition-all group border-2 border-transparent hover:border-red-100 active:scale-95"
        >
        ...
        <NuxtLink
          to="/settings"
          class="p-4 rounded-3xl flex items-center gap-3 overflow-hidden cursor-pointer transition-all group border-4 border-transparent hover:bg-slate-50 hover:border-slate-100"
    ...
    <nav
      class="md:hidden fixed bottom-0 left-0 right-0 bg-white border-t-4 border-slate-100 px-2 py-3 z-[100] flex justify-around items-center"
    >
      <NuxtLink
        v-for="item in navItems"
        :key="item.path"
      ...
      <NuxtLink
        ...
        :class="[isActive('/settings') ? 'text-brand-green translate-y-[-4px]' : 'text-slate-400']"
      >
        <div
          class="w-6 h-6 rounded-lg bg-brand-blue/10 flex items-center justify-center text-xs overflow-hidden"
        >
          <template v-if="userStore.user?.avatar && userStore.user.avatar.startsWith('data:image')">
            <img :src="userStore.user.avatar" class="w-full h-full object-cover" />
          </template>
          <template v-else>
            {{ userStore.user?.avatar || '🐍' }}
    ...
    <!-- Main Content Area -->
    <main class="flex-grow flex flex-col h-screen overflow-hidden overflow-y-auto custom-scrollbar">
      <!-- Header (Compact on Mobile) -->
      <header
        class="sticky top-0 h-20 md:h-24 bg-white/80 backdrop-blur-md border-b-4 border-slate-50 px-6 lg:px-12 flex items-center justify-between shrink-0 relative z-40"
      >
        ...
        <div class="flex items-center gap-3 md:gap-6">
          <div class="hidden lg:flex relative">
            <input
              type="text"
              placeholder="Поиск..."
              class="bg-slate-50 border-2 border-slate-100 rounded-2xl px-6 py-2.5 pl-10 font-bold text-sm text-slate-600 outline-none focus:border-brand-green transition-all w-48 xl:w-64"
            />
            <span class="absolute left-4 top-1/2 -translate-y-1/2 text-slate-300 text-sm">🔍</span>
          </div>

          <button
            class="w-10 h-10 md:w-12 md:h-12 bg-white rounded-xl md:rounded-2xl flex items-center justify-center border-2 border-slate-100 shadow-sm hover:translate-y-[-2px] hover:shadow-md transition-all relative shrink-0"
          >
            <span class="text-lg md:text-xl">🔔</span>
            <span
              class="absolute top-1.5 right-1.5 md:top-2 md:right-2 w-2.5 h-2.5 md:w-3 md:h-3 bg-brand-orange rounded-full border-2 border-white"
            ></span>
          </button>
        </div>
      </header>
...
const currentPageTitle = computed(() => {
  ...
  return navItems.value.find((item) => isActive(item.path))?.label || 'Zmeyka App'
})
