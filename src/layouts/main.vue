<template>
  <div class="relative">
    <div v-if="!isLoggingOut" class="min-h-screen bg-slate-50 flex overflow-hidden text-slate-900">
      ...
      >
        ...
        <div class="p-4 lg:p-8 flex items-center justify-center lg:justify-start gap-4">
          <NuxtLink to="/dashboard" class="flex items-center gap-4 group">
            ...
            <span
              class="hidden lg:block text-3xl font-black font-display text-slate-900 uppercase tracking-tighter"
              >Змейка</span
            >
          </NuxtLink>
        </div>
        ...
        <nav class="flex-grow px-4 lg:px-6 space-y-2 py-6 overflow-y-auto custom-scrollbar">
          ...
          >
            ...
            <span class="hidden lg:block text-sm uppercase tracking-widest font-display">{{
              item.label
            }}</span>
            <div
...
const handleSearch = () => {
  ...
  searchTimeout = setTimeout(async () => {
    try {
      ...
      // Ignore search errors or maybe set empty results
      searchResults.value = { courses: [], lessons: [] }
    }
  }, 300)
}
