    ...
    <div class="w-full max-w-lg relative z-10 space-y-8">
      ...
      <div class="bg-white p-10 rounded-[3rem] border-4 border-slate-100 shadow-cartoon space-y-8">
          ...
          <div class="flex items-center justify-between px-1">
            <label class="flex items-center gap-3 cursor-pointer group">
              <div class="relative w-7 h-7">
                <input v-model="form.rememberMe" type="checkbox" class="peer hidden" />
                <div
                  class="w-full h-full bg-slate-50 border-4 border-slate-100 rounded-lg transition-all peer-checked:bg-brand-green peer-checked:border-brand-green group-hover:border-slate-200"
                ></div>
                <div
                ...
                >
                  <span class="text-sm font-black mt-[-2px]">v</span>
                </div>
              </div>
              <span
                class="text-sm font-black text-slate-400 group-hover:text-slate-600 transition-colors uppercase tracking-widest"
                >Запомнить меня</span
              >
            </label>
          </div>
        ...
        <div class="grid grid-cols-2 gap-4">
          <button
            class="flex items-center justify-center gap-3 py-4 bg-white border-4 border-slate-100 rounded-2xl font-black text-slate-600 hover:bg-slate-50 transition-all shadow-sm"
          >
            <img
      ...
      <p class="text-center font-bold text-slate-500">
        ...
        <NuxtLink
          ...
          >Вылупись прямо сейчас!</NuxtLink
        >
      </p>
    </div>
  </div>
...
const handleLogin = async () => {
  ...
  try {
    ...
    // Backend uses camelCase for accessToken
    userStore.setUser(response.user, response.accessToken, form.rememberMe)
    router.push('/dashboard')
  } catch (err: unknown) {
    const errorData = err as { data?: { message?: string }; message?: string }
    error.value = errorData.data?.message || errorData.message || 'Ошибка входа. Проверь данные!'
  } finally {
    loading.value = false
  }
}
